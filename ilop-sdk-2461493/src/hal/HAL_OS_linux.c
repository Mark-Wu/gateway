...
static char DEMO_CASE_PRODUCT_KEY[PRODUCT_KEY_MAXLEN] = {"a1nTI8Slupd"};
static char DEMO_CASE_DEVICE_NAME[DEVICE_NAME_MAXLEN] = {"wHzYRHnDpyVwUwfk5TAa"};
static char DEMO_CASE_DEVICE_SECRET[DEVICE_SECRET_MAXLEN] = {"Fgcyxv1kroPBebKjDznockKZ4906fApb"};
...
{
    char temp[PRODUCT_KEY_MAXLEN];
    char temp1[DEVICE_NAME_MAXLEN];

    memset(device_id, 0x0, DEVICE_ID_MAXLEN);
    memset(temp,0x00,PRODUCT_KEY_MAXLEN);
    memset(temp1,0x00,DEVICE_NAME_MAXLEN);

    if((load_uci_item(UCI_GATEWAY_PK,temp) != 0) || (load_uci_item(UCI_GATEWAY_DN,temp1) != 0) ){
        HAL_Snprintf(device_id, DEVICE_ID_MAXLEN, "%s.%s", DEMO_CASE_PRODUCT_KEY, DEMO_CASE_DEVICE_NAME);

    }else{
        HAL_Snprintf(device_id, DEVICE_ID_MAXLEN, "%s.%s", temp, temp1);
    }
...
{
    char temp[DEVICE_NAME_MAXLEN];

    memset(temp,0x00,DEVICE_NAME_MAXLEN);
    memset(device_name, 0x0, DEVICE_NAME_MAXLEN);
    if(load_uci_item(UCI_GATEWAY_DN,temp) != 0){       
        HAL_Snprintf(device_name, DEVICE_NAME_MAXLEN, "%s", DEMO_CASE_DEVICE_NAME);
    }else{
        HAL_Snprintf(device_name, DEVICE_NAME_MAXLEN, "%s", temp);
    }
...
{
    char temp[DEVICE_SECRET_MAXLEN];
    
    memset(temp,0x00,DEVICE_SECRET_MAXLEN);   
    memset(device_secret, 0x0, DEVICE_SECRET_MAXLEN);

    if(load_uci_item(UCI_GATEWAY_DS,temp) != 0){       
        HAL_Snprintf(device_secret, DEVICE_SECRET_MAXLEN, "%s", DEMO_CASE_DEVICE_SECRET);
    }else{
        HAL_Snprintf(device_secret, DEVICE_SECRET_MAXLEN, "%s", temp);
    }
